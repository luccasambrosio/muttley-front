...
export default function CheckoutQRCodePage({ params }: PageProps) {
    ...
    async function iniciarDados() {
      try {
        ...
        if (dados.tokenCheckoutDinamico) {
          ...
          setUseEstatico(true);
          setTokenAtual(dados.tokenCheckoutEstatico || "SEM_TOKEN");
        }
      } catch (e) {
  ...
  if (loading) return <div className="min-h-screen bg-gray-50"><Header /><div className="text-center py-20">Carregando...</div></div>;

  // ---> A MÁGICA DO LINK <---
  // Pega o endereço base do site (ex: http://localhost:3000) e adiciona a rota do Aluno
  const baseUrl = typeof window !== "undefined" ? window.location.origin : "";
  const linkCheckout = `${baseUrl}/checkout-aluno/${id}?token=${tokenAtual}`;
  
  // Codifica o Link Inteiro no QR Code
