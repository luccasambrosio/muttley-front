...
import { PlusCircle, Calendar, Edit, Trash2, Ticket, Users, QrCode, CheckCircle, Smartphone } from "lucide-react";

export default function EventosPage() {
  const router = useRouter();
  const [eventos, setEventos] = useState<any[]>([]);
  // Adicionado o CPF no estado do usuário
  ...
  // Estados para o Modal de Ingresso do Aluno
  ...
  const verificarSessao = () => {
    ...
    if (token && userStr) {
      try {
        ...
        // Salvamos o CPF (sub) para gerar o QR Code do Ingresso
  ...
  const confirmarInscricaoDeslogado = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Inscrição realizada com sucesso! Um comprovante foi enviado ao seu e-mail.");
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        {/* Modal de Gestão do Evento (Admin/Gestor) */}
        ...
        {/* Modal de Inscrição Rápida (Aluno Deslogado) */}
        ...
        {/* Modal do Ingresso com QR Code (Aluno Logado) */}
        <Modal isOpen={isIngressoModalOpen} onClose={() => setIsIngressoModalOpen(false)} title="Seu Ingresso (Check-in)">
          <div className="text-center p-4">
            ...
            <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-inner inline-block mb-4">
              {/* Gera o QR Code com o CPF extraído do Token JWT */}
              {usuario?.cpf ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img 
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${usuario.cpf}`} 
                  alt="QR Code do Ingresso" 
                  className="w-48 h-48 select-none"
                />
              ) : (
                <div className="w-48 h-48 flex items-center justify-center text-gray-400">CPF não encontrado</div>
              )}
            </div>
