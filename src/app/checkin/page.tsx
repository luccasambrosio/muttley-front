...
export default function CheckInPage() {
  ...
  // Lógica da Câmera (só liga se escolher a aba "CAMERA" e tiver um evento selecionado)
  useEffect(() => {
    if (modo !== "CAMERA" || eventoSelecionado === "") {
    ...
    html5QrCode.start(
      ...
      (decodedText) => {
        // Se a câmera leu, congela ela e processa o CPF lido (Assumindo que o QR do aluno tem o CPF dele)
        if (!isLoading) {
      ...
      setMensagem({ texto: "Erro ao acessar câmera. Verifique as permissões.", tipo: "erro" });
      setModo("MANUAL");
    });
  ...
  }, [modo, eventoSelecionado]);

  const processarCheckIn = async (cpf: string) => {
    if (!eventoSelecionado || !cpf) return;
    
    setIsLoading(true);
    setMensagem({ texto: "", tipo: "" });

    try {
      await inscricaoService.checkIn(Number(eventoSelecionado), cpf);
      setMensagem({ texto: `Check-in de ${cpf} realizado com sucesso!`, tipo: "sucesso" });
      setCpfManual("");
      
      // Limpa a mensagem de sucesso depois de 3 segundos para ler o próximo
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
          ...
          <div className="p-6">
            ...
            <div className="mb-6 bg-gray-50 p-4 rounded-xl border">
              <label className="block text-sm font-bold text-gray-700 mb-2">Selecione o Evento Atual:</label>
              <select 
                className="w-full p-3 border rounded-lg bg-white outline-none focus:ring-2 focus:ring-blue-500 font-medium"
                value={eventoSelecionado}
                onChange={(e) => { setEventoSelecionado(Number(e.target.value)); setMensagem({texto:"", tipo:""}); }}
              >
                <option value="" disabled>-- Escolha a Palestra --</option>
                {eventos.map(ev => (
            ...
            {eventoSelecionado === "" ? (
              <div className="text-center py-10 text-gray-400 font-medium">
                Selecione um evento acima para liberar as opções de Check-in.
              </div>
            ) : (
              <>
                {/* ABAS: CAMERA VS MANUAL */}
                <div className="flex border-b mb-6">
                  <button onClick={() => setModo("MANUAL")} className={`flex-1 py-3 text-sm font-bold flex items-center justify-center gap-2 ${modo === "MANUAL" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-500"}`}>
                    <Keyboard className="w-4 h-4" /> Digitar CPF
                  </button>
                ...
                {modo === "MANUAL" && (
                  <form onSubmit={(e) => { e.preventDefault(); processarCheckIn(cpfManual); }} className="space-y-4">
                    <div>
                ...
                {modo === "CAMERA" && (
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-xs text-gray-500 mt-4 text-center">
                      Aponte a câmera para o QR Code de identificação do aluno.
                    </p>
                  </div>
                )}
              </>
