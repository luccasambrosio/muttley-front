...
import { eventoService } from "@/services/eventoService"; // <-- Adicionado o serviço de eventos
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-gradient-to-br from-purple-700 to-blue-600 rounded-3xl p-8 shadow-xl text-white mb-10 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden">
          ...
          <div className="flex-1 w-full text-center md:text-left z-10">
            ...
            <div className="bg-white/10 p-4 rounded-2xl backdrop-blur-md border border-white/20">
              ...
              <div className="w-full bg-black/20 rounded-full h-3 overflow-hidden">
                <div 
                  className="bg-gradient-to-r from-yellow-300 to-yellow-500 h-3 rounded-full transition-all duration-1000 relative" 
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center">
            ...
            <span className="text-3xl font-black text-gray-900">{inscricoesConcluidas.length}</span>
            <span className="text-xs font-bold text-gray-500 uppercase tracking-widest mt-1">Eventos Concluídos</span>
          </div>
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center">
            <Star className="w-8 h-8 text-yellow-500 mb-2" />
            <span className="text-3xl font-black text-gray-900">{pontos}</span>
            <span className="text-xs font-bold text-gray-500 uppercase tracking-widest mt-1">Total de XP</span>
          </div>
        </div>

        {/* GALERIA DE CERTIFICADOS */}
        <h2 className="text-2xl font-extrabold text-gray-900 mb-6 flex items-center gap-2">
          <Award className="w-6 h-6 text-purple-600" /> Seus Certificados
        </h2>

        {inscricoesConcluidas.length === 0 ? (
          <div className="bg-white border border-dashed border-gray-300 rounded-2xl p-12 text-center text-gray-500 font-medium">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {inscricoesConcluidas.map((insc: any) => (
              <div key={insc.id} className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                  <Award className="w-24 h-24" />
                </div>
                <div className="relative z-10">
                  {/* Utilização do operador ? para navegação segura */}
                  <h3 className="font-black text-lg text-gray-900 leading-tight mb-2 pr-12">
